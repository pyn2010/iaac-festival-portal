-- Safe continuation for a live database where 202608120001_group_account_model.sql
-- was partially applied and public.app_role = 'group' was later added separately.
--
-- This migration intentionally starts from the work immediately after:
--   update public.profiles set role = 'group'::public.app_role where role::text = 'participant';
-- and makes each remaining schema/RLS/function step idempotent so it can also be
-- run when some later statements from 202608120001/202608120002/202608130001
-- already succeeded.

-- Ensure dependent enum/table/column objects exist if the earlier partial run did
-- not reach them. These statements preserve existing rows and definitions.
do $$
begin
  if not exists (select 1 from pg_type where typnamespace = 'public'::regnamespace and typname = 'group_id_status') then
    create type public.group_id_status as enum ('available','claimed','disabled','replaced');
  end if;

  if not exists (select 1 from pg_type where typnamespace = 'public'::regnamespace and typname = 'document_scope') then
    create type public.document_scope as enum ('official','group');
  end if;
end;
$$;

create table if not exists public.group_ids (
...
create table if not exists public.group_id_events (
  id uuid primary key default gen_random_uuid(),
...
-- Complete the failed role conversion after the enum value is known to be committed.
update public.profiles
set role = 'group'::public.app_role,
    updated_at = now()
where role::text = 'participant';

-- Complete indexes/constraints after the failed role update. Constraint creation is
-- guarded because ALTER TABLE ... ADD CONSTRAINT has no IF NOT EXISTS form.
...
begin
  if not exists (
    ...
    where conrelid = 'public.applications'::regclass
      and conname = 'applications_group_id_key'
  ) then
...
-- Preserve and complete the 20 production Group IDs. Existing rows are untouched.
...
-- Preserve the organiser test Group ID and ensure it stays reserved/marked.
...
create or replace function public.current_group_id() returns uuid
language sql
stable
security definer
...
-- Final Auth trigger: creates the profile for both manually-created Admin users and
-- Group signups, securely assigns the role server-side, and claims the metadata
-- Group ID during email-confirmed signup without relying on a browser session.
...
-- Backfill profiles for Auth users created while the trigger was absent/incomplete.
...
create or replace function public.admin_replace_group_id(old_group_id uuid, new_code text, reason text default null) returns uuid
language plpgsql
security definer
set search_path = public
as $$
...
begin
  ...
  insert into public.group_ids(code, notes)
  values (upper(trim(new_code)), reason)
  on conflict (code) do update set notes = coalesce(public.group_ids.notes, excluded.notes)
  returning id into new_group_id;

  update public.group_ids
  set status = 'replaced', replaced_by = new_group_id, notes = coalesce(reason, notes), updated_at = now()
  where id = old_group_id and status <> 'claimed';

  update public.group_ids set replacement_for = old_group_id where id = new_group_id;

  insert into public.group_id_events(group_id, actor_id, event_type, details)
  values (old_group_id, auth.uid(), 'replaced', jsonb_build_object('replacement_group_id', new_group_id, 'reason', reason));

  return new_group_id;
end;
$$;

create or replace function public.admin_disable_group_id(target_group_id uuid, reason text default null) returns void
language plpgsql
security definer
set search_path = public
as $$
begin
  ...
  update public.group_ids
  set status = 'disabled', notes = coalesce(reason, notes), updated_at = now()
  where id = target_group_id and status <> 'claimed';

  insert into public.group_id_events(group_id, actor_id, event_type, details)
  values (target_group_id, auth.uid(), 'disabled', jsonb_build_object('reason', reason));
end;
$$;
...
-- Replace obsolete participant-era policies and install final Admin/Group policies.
...
drop policy if exists "documents admin update" on public.event_documents;
...
do $$
begin
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'profiles' and policyname = 'profiles own group or admin read') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'profiles' and policyname = 'profiles own update no role escalation') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'profiles' and policyname = 'profiles self insert group only') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'applications' and policyname = 'applications group scoped or admin') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'event_documents' and policyname = 'documents scoped to official group or admin') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'event_documents' and policyname = 'documents admin insert') then
    create policy "documents admin insert" on public.event_documents for insert with check (public.current_role() = 'admin');
  end if;

  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'event_documents' and policyname = 'documents admin delete') then
    create policy "documents admin delete" on public.event_documents for delete using (public.current_role() = 'admin');
  end if;

  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'event_documents' and policyname = 'documents admin update') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'group_ids' and policyname = 'group ids admin read all group read own') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'group_ids' and policyname = 'group ids admin insert') then
    create policy "group ids admin insert" on public.group_ids for insert with check (public.current_role() = 'admin');
  end if;

  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'group_ids' and policyname = 'group ids admin update') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'group_id_events' and policyname = 'group id events admin or own group read') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'group_id_events' and policyname = 'group id events admin insert') then
    create policy "group id events admin insert" on public.group_id_events for insert with check (public.current_role() = 'admin');
  end if;

  if not exists (select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'scoped document downloads') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'admin document uploads') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'storage' and tablename = 'objects' and policyname = 'admin document deletes') then
